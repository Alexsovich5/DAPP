...
def create_user(user_data):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            logger.error(
                f"Failed to create user {user_data['username']}: {response.status_code} - {response.text}"
            )
            return None
...
def login_user(username, password):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"Failed to login user {username}: {response.status_code} - {response.text}"
            )
            return None
    except Exception as e:
...
def create_profile(token, profile_data):
    ...
    try:
        ...
        response = requests.post(
            f"{BASE_URL}/profiles", json=profile_data, headers=headers
        )
        if response.status_code == 201:
