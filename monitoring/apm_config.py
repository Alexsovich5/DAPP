...
class APMService:
        ...
        try:
            ...
            for metric in metrics_data:
                nr_metric = {
                    ...
                    "value": metric["value"],
                    "timestamp": int(
                        datetime.fromisoformat(metric["timestamp"]).timestamp() * 1000
                    ),
            ...
            for metric in metrics_data:
                dd_metric = {
                    ...
                    "points": [
                        [
                            int(
                                datetime.fromisoformat(metric["timestamp"]).timestamp()
                            ),
                            metric["value"],
                        ]
                    ],
    ...
    async def _get_recent_metric_values(
        self, metric_name: str, duration_minutes: int
    ) -> List[float]:
    ...
    async def _trigger_alert(self, rule: AlertRule, current_value: float):
        ...
        try:
            ...
            logger.warning(
                f"Alert triggered: {rule.name} - {current_value} {rule.comparison} {rule.threshold}"
            )
    ...
    async def get_metrics_summary(self, duration_hours: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            key_metrics = [
                ...
                "matches_created_rate",
                "user_sessions_active",
            ]

            for metric in key_metrics:
                values = await self._get_recent_metric_values(
                    metric, duration_hours * 60
                )
