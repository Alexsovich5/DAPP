...
# Add app to path - this must be done before importing app modules
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "app"))

# Now import app modules
from app.services.advanced_soul_matching import AdvancedSoulMatchingService  # noqa: E402
...
def test_integration_scenario():
    ...
    # enhanced_service = EnhancedMatchQualityService()  # Service tested above
...
def test_performance_benchmarks():
    ...
    assert (
        init_time < 1.0
    ), f"Service initialization took {init_time:.2f}s (should be < 1.0s)"
    ...
    ), f"100 computations took {computation_time:.2f}s (should be < 0.5s)"
