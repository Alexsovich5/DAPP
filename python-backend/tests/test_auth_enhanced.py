...
class TestJWTTokenManagement:
    ...
    def test_invalid_token_formats(self, client):
        ...
        for invalid_token in invalid_tokens:
            headers = (
                {"Authorization": f"Bearer {invalid_token}"} if invalid_token else {}
            )
...
class TestRateLimiting:
    ...
    def test_rate_limit_reset_after_timeout(self):
        ...
        def is_rate_limited():
            now = datetime.now()
            if (now - rate_limiter["last_attempt"]).total_seconds() > (
                rate_limiter["window_minutes"] * 60
            ):
    ...
    def test_password_strength_validation(self):
        ...
        for strong_pass in strong_passwords:
            assert validate_password_strength(
                strong_pass
            ), f"Password '{strong_pass}' should be accepted"
...
class TestSessionSecurity:
    ...
    def test_token_payload_security(self):
        ...
        safe_data = {"sub": "user@example.com", "user_id": 1}
    ...
    def test_user_isolation_in_connections(self, client, matching_users, db_session):
        ...
        if create_response.status_code == status.HTTP_201_CREATED:
            ...
            response2 = client.get(
                f"/api/v1/connections/{connection_id}", headers=headers2
            )
