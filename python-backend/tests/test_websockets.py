...
from app.services.realtime_connection_manager import (
    RealtimeConnectionManager as ConnectionManager,
)
...
class TestWebSocketConnection:
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_connection_manager_connect_user(
        self, connection_manager, db_session, test_user
    ):
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_connection_manager_disconnect_user(
        self, connection_manager, db_session, test_user
    ):
...
class TestRealtimeMessaging:
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    def test_message_delivery_to_recipient(
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    def test_message_persistence_and_realtime(
        self, ws_client, realtime_service, soul_connection_data
    ):
...
class TestTypingIndicators:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    def test_multiple_users_typing_indicators(
        self, realtime_service, soul_connection_data
    ):
...
class TestPresenceStatus:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    def test_presence_notification_to_connections(
        self, realtime_service, soul_connection_data
    ):
...
class TestRealtimeNotifications:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    def test_new_connection_notification(
        self, ws_client, realtime_service, authenticated_user
    ):
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    def test_revelation_shared_notification(
        self, realtime_service, soul_connection_data
    ):
        ...
        with patch.object(realtime_service, "connection_manager", mock_manager):
            realtime_service.notify_revelation_shared(
                connection.id, user1.id, revelation_notification
            )
    ...
    def test_photo_consent_notification(self, realtime_service, soul_connection_data):
        ...
        with patch.object(
            realtime_service, "connection_manager", MagicMock()
        ) as mock_manager:
            mock_manager.send_personal_message = AsyncMock()

            realtime_service.notify_photo_consent(
                connection.id, user1.id, consent_notification
            )
    ...
    def test_notification_queuing_for_offline_users(self, realtime_service):
        ...
        result = realtime_service.send_notification_with_queue(
            offline_user_id, notification
        )
        ...
        queued_notifications = realtime_service.get_queued_notifications(
            offline_user_id
        )
...
class TestWebSocketSecurity:
    ...
    def test_websocket_jwt_validation(self, ws_client):
        ...
        invalid_tokens = ["invalid.jwt.token", "expired_token", "", None]
    ...
    @pytest.mark.asyncio
    @pytest.mark.security
    def test_websocket_connection_isolation(
        self, realtime_service, soul_connection_data
    ):
...
class TestWebSocketPerformance:
    ...
    def test_concurrent_websocket_connections(
        self, connection_manager, performance_config
    ):
        ...
        with patch.object(realtime_service, "connection_manager", MagicMock()):
            ...
            for i in range(10):
                realtime_service.send_message_to_connection(
                    {**message_data, "content": f"Performance message {i}"}
                )
