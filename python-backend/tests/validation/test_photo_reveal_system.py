...
async def test_photo_reveal_workflow():
    ...
    try:
        ...
        print(
            f"   ✅ Created users: {user1.first_name} (ID: {user1.id}) & {user2.first_name} (ID: {user2.id})"
        )
        ...
        print(
            f"   ✅ Created connection (ID: {connection.id}) with {connection.compatibility_score}% compatibility"
        )
        ...
        revelations = [
            {
                "day": 1,
                "user": user1,
                "content": "I value deep emotional connection above all",
            },
            {
                "day": 2,
                "user": user2,
                "content": "My biggest dream is to travel the world with someone special",
            },
            {
                "day": 3,
                "user": user1,
            ...
            },
            {
                "day": 5,
                "user": user1,
                "content": "I believe vulnerability is the key to intimacy",
            },
        ]
        ...
        print(
            f"   Revelations completed: {status.revelations_completed}/{status.min_revelations_required}"
        )
        ...
        final_status = await photo_reveal_service.get_photo_reveal_status(
            connection.id, db
        )
        ...
        # Cleanup test data
        try:
            db.query(DailyRevelation).filter(
                DailyRevelation.connection_id == connection.id
            ).delete()
            db.query(PhotoRevealTimeline).filter(
                PhotoRevealTimeline.connection_id == connection.id
            ).delete()
            db.query(UserPhoto).filter(
                UserPhoto.user_id.in_([user1.id, user2.id])
            ).delete()
