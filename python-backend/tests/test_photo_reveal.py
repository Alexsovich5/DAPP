...
class TestPhotoRevealConsent:
    ...
    def test_photo_reveal_status_enum(self):
        ...
        expected_statuses = [
            "hidden",
            "consent_requested",
            "consented",  # Added compatibility alias
            "mutual_consent",
            "revealed",
            "declined",
        ]
        ...
        # Should have exactly these statuses for privacy control (updated count)
        assert len(actual_statuses) == 6
...
class TestPhotoRevealAPI:
    ...
    def test_photo_reveal_privacy_protection(self, client, matching_users):
        ...
        token2 = client.post(
            "/api/v1/auth/login",
            data={"username": user2.email, "password": "testpass123"},
        ).json()["access_token"]
        ...
        if connection_response.status_code == status.HTTP_201_CREATED:
            ...
            view_response = client.get(
                f"/api/v1/photo-reveal/view/{connection_id}", headers=headers2
            )
...
class TestPhotoRevealSecurity:
    ...
    def test_photo_storage_security(self, photo_reveal_service):
        ...
        stored_info = photo_reveal_service.store_photo_securely(
            photo_data=photo_data, connection_id=connection_id, user_id=user_id
        )
...
class TestPhotoRevealBusinessLogic:
    ...
    @pytest.mark.photo_reveal
    def test_photo_processing_performance(
        self, photo_reveal_service, performance_config
    ):
        ...
        result = photo_reveal_service.process_photo_for_reveal(
            photo_data=large_photo_data, connection_id=1, user_id=1
        )
...
class TestPhotoRevealIntegration:
    ...
    def test_photo_reveal_affects_matching_algorithm(
        ...
        updated_preferences = photo_reveal_service.get_updated_matching_preferences(
            users[0].id
        )
        ...
        assert (
            updated_preferences["photo_reveal_comfort_level"] >= 7
        )  # High comfort after success
