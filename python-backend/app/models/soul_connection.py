...
class ConnectionStage(str, enum.Enum):
    ...
    REVELATION_SHARING = "revelation_sharing"
...
class SoulConnection(Base):
    ...
    stage_progression_dates = Column(
        JSON, nullable=True
    ...
    )  # If ended: mutual, timeout, reported, etc.
    ...
    photo_timeline = relationship(
        "PhotoRevealTimeline", back_populates="connection", uselist=False
    )
