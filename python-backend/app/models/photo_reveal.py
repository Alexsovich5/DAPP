...
class PhotoRevealStage(Enum):
    ...
    CONSENTED = "consented"  # User has given consent (compatibility alias)
...
class UserPhoto(Base):
    ...
    privacy_level = Column(
        SQLEnum(PhotoPrivacyLevel), default=PhotoPrivacyLevel.COMPLETELY_HIDDEN
    )
    ...
    def get_reveal_url(
        self, privacy_level: PhotoPrivacyLevel, requester_id: int
    ) -> Optional[str]:
...
class PhotoRevealTimeline(Base):
    ...
    connection_id = Column(
        Integer, ForeignKey("soul_connections.id"), nullable=False, unique=True
    )
    ...
    def calculate_days_until_reveal(self) -> int:
        ...
        if self.connection_started_at:
            target_date = self.connection_started_at + timedelta(
                days=self.revelation_cycle_days
            )
...
class PhotoRevealRequest(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    timeline_id = Column(
        Integer, ForeignKey("photo_reveal_timelines.id"), nullable=False
    )
    ...
    requested_privacy_level = Column(
        SQLEnum(PhotoPrivacyLevel), default=PhotoPrivacyLevel.FULLY_REVEALED
    )
    granted_privacy_level = Column(
        SQLEnum(PhotoPrivacyLevel)
...
class PhotoRevealEvent(Base):
    ...
    )  # consent_requested, consent_granted, photo_revealed, etc.
...
class PhotoModerationLog(Base):
    ...
    # Moderation details
    moderation_type = Column(
        String, nullable=False
    )  # ai_scan, manual_review, user_report
