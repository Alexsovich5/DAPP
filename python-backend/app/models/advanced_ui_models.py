...
class UIExperienceProfile(Base):
    ...
    pixel_density_optimization = Column(
    ...
    emotional_design_preference = Column(
        String, default="balanced"
    )  # warm, cool, balanced, vibrant
    animation_preference = Column(
        String, default="standard"
    )  # minimal, standard, enhanced, dynamic
    ...
    performance_priority = Column(
        String, default="balanced"
    )  # performance, visual_quality, balanced
    ...
    accessibility_profile = relationship(
        "AccessibilityProfile", back_populates="ui_profile", uselist=False
    )
...
class InteractionAnimation(Base):
    ...
    ui_profile_id = Column(
        Integer, ForeignKey("ui_experience_profiles.id"), nullable=False
    )
...
class AccessibilityProfile(Base):
    ...
    visual_impairment_type = Column(
        String, nullable=True
    )  # none, low_vision, blind, color_blind
    contrast_requirement = Column(String, default="normal")  # normal, high, ultra_high
    text_size_multiplier = Column(Float, default=1.0)
    color_blind_type = Column(
        String, nullable=True
    )  # protanopia, deuteranopia, tritanopia

    # Motor accessibility
    motor_limitation_type = Column(
        String, nullable=True
    )  # none, fine_motor, gross_motor, tremor
    ...
    cognitive_load_preference = Column(
        String, default="standard"
    )  # minimal, standard, enhanced
    information_density_preference = Column(
        String, default="medium"
    )  # low, medium, high
    ...
    # Relationships
    ui_profile = relationship(
        "UIExperienceProfile", back_populates="accessibility_profile"
    )
...
class UserJourney(Base):
    ...
    )  # onboarding, matching, messaging, etc.
