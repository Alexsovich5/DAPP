...
class UserProfile(Base):
    ...
    partner_compatibility_scores = relationship(
        "CompatibilityPrediction",
        foreign_keys="[CompatibilityPrediction.user2_profile_id]",
        back_populates="partner_profile",
    )
    recommendations = relationship(
        "PersonalizedRecommendation", back_populates="user_profile"
    )
...
class MLModel(Base):
    ...
    architecture = Column(
        String, nullable=True
    )  # neural_network, gradient_boosting, etc.
    ...
    compatibility_predictions = relationship(
        "CompatibilityPrediction", back_populates="model"
    )
...
class CompatibilityPrediction(Base):
    ...
    user_feedback_score = Column(
        Float, nullable=True
    )  # User rating of prediction quality
    ...
    def get_compatibility_insights(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "breakdown": {
                ...
                "communication": round(
                    (self.communication_compatibility or 0) * 100, 1
                ),
                "lifestyle": round((self.lifestyle_compatibility or 0) * 100, 1),
            },
            "predictions": {
                "conversation_quality": round(
                    (self.conversation_quality_prediction or 0) * 100, 1
                ),
...
class PersonalizedRecommendation(Base):
    ...
    )  # profile_suggestion, conversation_starter, etc.
    ...
    invalidation_reason = Column(
        String, nullable=True
    )  # Why recommendation became invalid
...
class ModelPrediction(Base):
    ...
    prediction_type = Column(
        String, nullable=False
    )  # compatibility, recommendation, etc.
