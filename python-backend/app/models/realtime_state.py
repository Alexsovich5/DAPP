...
class UserPresence(Base):
    ...
    active_connection = relationship(
        "SoulConnection", foreign_keys=[active_connection_id]
    )
    typing_connection = relationship(
        "SoulConnection", foreign_keys=[typing_in_connection]
    )
...
class AnimationEvent(Base):
    ...
    # User engagement
    user_interacted = Column(
        Boolean, default=False
    )  # Did user tap/interact during animation
    emotional_response = Column(
        String, nullable=True
    )  # Inferred from subsequent actions
