...
class UserUIProfile(Base):
    ...
    preferred_orientation = Column(
        String, default="portrait"
    )  # portrait, landscape, auto

    # Visual Preferences
    preferred_theme = Column(String, default="auto")  # light, dark, auto
    color_contrast_preference = Column(
        String, default="standard"
    )  # low, standard, high
    font_size_preference = Column(
        String, default="medium"
    )  # small, medium, large, extra-large
    animation_preference = Column(
        String, default="standard"
    )  # none, reduced, standard, enhanced

    # Layout Preferences
    layout_density = Column(
        String, default="comfortable"
    ...
    primary_interaction_method = Column(
        String, default="touch"
    )  # touch, mouse, keyboard, voice
    swipe_sensitivity = Column(
        Float, default=1.0
    )  # 0.5 = low, 1.0 = normal, 1.5 = high
    ...
    navigation_pattern = Column(
        String, default="explorer"
    )  # linear, explorer, focused, random
    ...
    personalization_events = relationship(
        "UIPersonalizationEvent", back_populates="ui_profile"
    )
    a_b_test_participations = relationship(
        "UIABTestParticipation", back_populates="ui_profile"
    )
    ...
    def update_personalization_score(
        self, new_interactions: int, satisfaction_score: float
    ):
        ...
        if self.total_interactions_tracked == 0:
            ...
            self.personalization_score = (
                self.personalization_score * (1 - weight) + satisfaction_score * weight
            )
...
class UIPersonalizationEvent(Base):
    ...
    personalization_type = Column(
        String, nullable=False
    )  # theme_change, layout_adjustment, etc.
    ...
    )  # behavioral_pattern, user_request, etc.
    ...
    interaction_improvement = Column(
        Float, nullable=True
    ...
    def measure_effectiveness(
        self, satisfaction: float, interaction_improvement: float
    ):
...
class UIABTestParticipation(Base):
    ...
    conversion_type = Column(
        String, nullable=True
    )  # registration, match, message, etc.
...
class UIAdaptiveComponent(Base):
    ...
    # Configuration
    is_active = Column(Boolean, default=True)
    adaptation_frequency = Column(
        String, default="real_time"
    )  # real_time, session, daily
...
class UIPersonalizationInsight(Base):
    ...
    insight_type = Column(
        String, nullable=False
    )  # pattern, preference, anomaly, opportunity
    insight_category = Column(
        String, nullable=False
    ...
    implementation_priority = Column(
        String, default="medium"
