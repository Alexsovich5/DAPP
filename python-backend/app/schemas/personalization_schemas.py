...
class ConversationStarterRequest(BaseModel):
    ...
    count: int = Field(
        default=5, ge=1, le=10, description="Number of starters to generate"
    )
...
class RevelationPromptRequest(BaseModel):
    ...
    connection_context: Dict[str, Any] = Field(
        ..., description="Connection context and progress"
    )
...
class SmartReplyRequest(BaseModel):
    ...
    reply_count: int = Field(
        default=3, ge=1, le=5, description="Number of reply suggestions"
    )
...
class UIPersonalizationRequest(BaseModel):
    ...
    current_context: Dict[str, Any] = Field(
        ..., description="Current app context and user state"
    )
    screen: str = Field(..., description="Current screen/page identifier")
    device_type: Optional[str] = Field(
        default="web", description="Device type (web, mobile, tablet)"
    )
...
class ContentFeedbackRequest(BaseModel):
    ...
    feedback_data: Dict[str, Any] = Field(
        ..., description="Feedback data including type, score, etc."
    )
...
class AlgorithmOptimizationRequest(BaseModel):
    ...
    optimization_type: str = Field(
        ...,
        description="Type of optimization (matching, content_generation, ui_adaptation)",
    )
