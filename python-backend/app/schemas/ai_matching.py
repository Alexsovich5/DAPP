...
class CompatibilityBreakdown(BaseModel):
    ...
    semantic_similarity: float = Field(
        ..., ge=0, le=100, description="Semantic text similarity score"
    )
    communication_style: float = Field(
        ..., ge=0, le=100, description="Communication compatibility score"
    )
    emotional_depth: float = Field(
        ..., ge=0, le=100, description="Emotional compatibility score"
    )
    life_goals: float = Field(
        ..., ge=0, le=100, description="Life goals alignment score"
    )
    personality_match: float = Field(
        ..., ge=0, le=100, description="Personality compatibility score"
    )
    interest_overlap: float = Field(
        ..., ge=0, le=100, description="Interest overlap score"
    )
...
class AICompatibilityAnalysis(BaseModel):
    ...
    ai_compatibility_score: float = Field(
        ..., ge=0, le=100, description="Overall AI compatibility score"
    )
    confidence_level: float = Field(
        ..., ge=0, le=100, description="Confidence in the analysis"
    )
    compatibility_breakdown: CompatibilityBreakdown
    unique_connection_potential: List[str] = Field(
        default=[], description="Unique factors for connection"
    )
    analysis_timestamp: datetime = Field(default_factory=datetime.utcnow)
    processing_time_ms: Optional[float] = Field(
        None, description="Analysis processing time in milliseconds"
    )
...
class ConversationStarter(BaseModel):
    ...
    category: str = Field(
        ...,
        description="Category of the starter (interests, values, personality, etc.)",
    )
    confidence_score: float = Field(
        ..., ge=0, le=1, description="Confidence in starter effectiveness"
    )
    personalization_factors: List[str] = Field(
        default=[], description="Factors used for personalization"
    )
...
class PersonalityInsight(BaseModel):
    ...
    aspect: str = Field(
        ..., description="Personality aspect (e.g., 'communication style')"
    )
...
class DeepCompatibilityAnalysis(BaseModel):
    ...
    relationship_potential: str = Field(
        ..., description="Assessment of relationship potential"
    )
    growth_opportunities: List[str] = Field(
        default=[], description="Areas for mutual growth"
    )
    potential_challenges: List[str] = Field(
        default=[], description="Potential relationship challenges"
    )
    recommended_activities: List[str] = Field(
        default=[], description="Recommended activities for connection"
    )
...
class AIModelStatus(BaseModel):
    ...
    training_data_size: int = Field(
        default=0, description="Number of profiles used for training"
    )
...
class AIMatchingConfig(BaseModel):
    ...
    compatibility_weights: Dict[str, float] = Field(
        default={
    ...
    privacy_mode: bool = Field(
        default=True, description="Ensure all processing is local"
    )
...
class SimilarityMatrix(BaseModel):
    ...
    user_ids: List[int]
    similarity_scores: List[List[float]] = Field(
        ..., description="NxN similarity matrix"
    )
...
class MatchingMetrics(BaseModel):
    ...
    high_compatibility_matches: int = Field(
        default=0, description="Matches with >80% compatibility"
    )
...
class AIMatchingError(BaseModel):
    ...
    error_type: str = Field(
        ..., description="Type of error (validation, processing, model, etc.)"
    )
    error_message: str = Field(..., description="Detailed error message")
    user_ids: Optional[List[int]] = Field(
        None, description="User IDs involved in failed analysis"
    )
    timestamp: datetime = Field(default_factory=datetime.utcnow)
    stack_trace: Optional[str] = Field(
        None, description="Error stack trace for debugging"
    )
