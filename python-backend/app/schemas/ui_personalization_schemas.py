...
class InteractionTrackingRequest(BaseModel):
    ...
    type: str = Field(
        ..., description="Type of interaction (click, scroll, swipe, etc.)"
    )
    ...
    screen_size: Optional[str] = Field(
        None, description="Screen dimensions (e.g., '375x667')"
    )
    ...
    time_since_last: Optional[float] = Field(
        None, description="Time since last interaction (seconds)"
    )
    duration: Optional[float] = Field(
        None, description="Interaction duration (seconds)"
    )
    coordinates: Optional[Dict[str, int]] = Field(
        None, description="Click/touch coordinates"
    )
    scroll_distance: Optional[int] = Field(
        None, description="Scroll distance in pixels"
    )
    swipe_direction: Optional[str] = Field(
        None, description="Swipe direction (up, down, left, right)"
    )
    gesture_data: Optional[Dict[str, Any]] = Field(
        None, description="Complex gesture data"
    )
    emotional_state: Optional[str] = Field(None, description="Current emotional state")
    connection_context: Optional[Dict[str, Any]] = Field(
        None, description="Connection context data"
    )
    feature_flags: Optional[Dict[str, Any]] = Field(
        None, description="Active feature flags"
    )
    render_time: Optional[float] = Field(
        None, description="Render time in milliseconds"
    )
...
class UIAdaptationRequest(BaseModel):
    ...
    preferences_override: Optional[Dict[str, Any]] = Field(
        None, description="Temporary preference overrides"
    )
...
class UIPersonalizationResponse(BaseModel):
    ...
    user_id: int
    personalizations: Dict[str, Any] = Field(
        ..., description="Personalization recommendations by category"
    )
...
class PersonalizationInsightResponse(BaseModel):
    ...
    impact_estimate: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Expected impact"
    )
    generated_at: datetime
    implemented: bool = Field(
        False, description="Whether the insight has been acted upon"
    )
...
class InteractionPattern(BaseModel):
    ...
    pattern_type: str = Field(
        ..., description="Type of pattern (navigation, timing, device)"
    )
...
class BehaviorAnalysis(BaseModel):
    ...
    analysis_period: str = Field(
        ..., description="Period of analysis (e.g., 'last_30_days')"
    )
    total_interactions: int
    ...
    engagement_level: str = Field(
        ..., description="Overall engagement level (low, medium, high)"
    )
...
class UITestVariant(BaseModel):
    ...
    expected_impact: float = Field(
        ..., ge=0.0, le=1.0, description="Expected improvement"
    )
...
class RealTimeAdaptationResponse(BaseModel):
    ...
    adaptations_applied: Dict[str, Any] = Field(
        ..., description="Specific adaptations applied"
    )
    trigger_reason: str = Field(..., description="What triggered this adaptation")
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence in adaptation quality"
    )
    expected_improvement: float = Field(
        ..., ge=0.0, le=1.0, description="Expected UX improvement"
    )
    rollback_available: bool = Field(
        True, description="Whether adaptation can be rolled back"
    )
...
class UIPreferencesUpdate(BaseModel):
    ...
    font_size_preference: Optional[str] = Field(
        None, description="Font size preference"
    )
    animation_preference: Optional[str] = Field(
        None, description="Animation preference"
    )
    layout_density: Optional[str] = Field(None, description="Layout density preference")
    color_contrast_preference: Optional[str] = Field(
        None, description="Color contrast preference"
    )
    haptic_feedback_intensity: Optional[float] = Field(None, ge=0.0, le=1.0)
    screen_reader_enabled: Optional[bool] = Field(
        None, description="Screen reader usage"
    )
    keyboard_navigation_primary: Optional[bool] = Field(
        None, description="Primary keyboard navigation"
    )
    high_contrast_enabled: Optional[bool] = Field(
        None, description="High contrast mode"
    )
    reduce_motion_enabled: Optional[bool] = Field(
    ...
    def validate_animation(cls, v):
        if v and v not in ["none", "reduced", "standard", "enhanced"]:
            raise ValueError(
                "Animation preference must be none, reduced, standard, or enhanced"
            )
        return v
...
class UIPersonalizationConfig(BaseModel):
    ...
    feedback_frequency: str = Field(
        default="adaptive", description="How often to ask for feedback"
    )
    auto_apply_high_confidence: bool = Field(default=True)
    enable_a_b_testing: bool = Field(default=True)
    privacy_level: str = Field(
        default="standard", description="Data collection privacy level"
    )
...
class UIMetrics(BaseModel):
    ...
    page_load_time: Optional[float] = Field(
        None, description="Average page load time (ms)"
    )
    interaction_response_time: Optional[float] = Field(
        None, description="Average interaction response time (ms)"
    )
    ...
    task_completion_rate: float = Field(
        ..., ge=0.0, le=1.0, description="Task completion rate"
    )
    bounce_rate: float = Field(..., ge=0.0, le=1.0, description="Bounce rate")
    session_duration: Optional[float] = Field(
        None, description="Average session duration (minutes)"
    )
    interactions_per_session: Optional[float] = Field(
        None, description="Average interactions per session"
    )
    ...
    mobile_usability_score: float = Field(
        ..., ge=0.0, le=1.0, description="Mobile usability score"
    )
