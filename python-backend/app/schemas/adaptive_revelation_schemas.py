...
class AdaptiveRevelationRequest(BaseModel):
    ...
    revelation_day: int = Field(
        ..., ge=1, le=7, description="Day in the revelation cycle (1-7)"
    )
    count: int = Field(
        default=3, ge=1, le=5, description="Number of prompts to generate"
    )
    preferences: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional preferences"
    )
...
class AdaptiveRevelationResponse(BaseModel):
    ...
    timing_recommendation: Optional[Dict[str, Any]] = Field(
        default=None, description="Optimal timing info"
    )
...
class RevelationThemeResponse(BaseModel):
    ...
    requires_high_compatibility: bool = Field(
        ..., description="Whether theme requires high compatibility"
    )
    emotional_intensity: float = Field(
        ..., ge=0.0, le=1.0, description="Emotional intensity level"
    )
...
class RevelationTimingResponse(BaseModel):
    ...
    optimal_day_time: str = Field(
        ..., description="Best time period (morning, afternoon, evening)"
    )
    ...
    next_optimal_time: Optional[datetime] = Field(
        default=None, description="Next specific optimal time"
    )
...
class RevelationFeedbackRequest(BaseModel):
    ...
    helpful_score: float = Field(
        ..., ge=1.0, le=5.0, description="How helpful was the prompt (1-5)"
    )
    engagement_score: float = Field(
        ..., ge=1.0, le=5.0, description="How engaging was the prompt (1-5)"
    )
    emotional_resonance: float = Field(
        ..., ge=1.0, le=5.0, description="Emotional impact (1-5)"
    )
    timing_appropriateness: float = Field(
        ..., ge=1.0, le=5.0, description="Timing quality (1-5)"
    )
    comments: Optional[str] = Field(
        default=None, description="Optional feedback comments"
    )
...
class RevelationAnalyticsResponse(BaseModel):
    ...
    connection_id: int = Field(..., description="Connection ID")
    completed_revelation_days: int = Field(
        ..., description="Number of revelation days completed"
    )
    total_revelations_shared: int = Field(
        ..., description="Total revelations shared by user"
    )
    average_words_per_revelation: float = Field(
        ..., description="Average word count per revelation"
    )
    theme_distribution: Dict[str, int] = Field(
        ..., description="Distribution of themes used"
    )
    engagement_trend: str = Field(..., description="Overall engagement trend")
    most_successful_themes: List[str] = Field(
        ..., description="Themes with highest engagement"
    )
    overall_depth_score: float = Field(..., description="Average depth/intensity score")
    personalization_effectiveness: float = Field(
        ..., ge=0.0, le=1.0, description="How well personalization works"
    )
    next_recommended_theme: Optional[str] = Field(
        default=None, description="Next recommended theme"
    )
...
class RevelationInsight(BaseModel):
    ...
    insight_type: str = Field(
        ..., description="Type of insight (performance, suggestion, warning)"
    )
...
class RevelationOptimizationRequest(BaseModel):
    ...
    optimization_goals: List[str] = Field(
        ..., description="What to optimize (engagement, depth, timing)"
    )
...
class RevelationOptimizationResponse(BaseModel):
    ...
    improved_prompts: List[AdaptiveRevelationResponse] = Field(
        ..., description="Optimized prompts"
    )
    expected_improvements: Dict[str, float] = Field(
        ..., description="Expected improvement percentages"
    )
...
class RealTimeRevelationUpdate(BaseModel):
    ...
    update_type: str = Field(
        ..., description="Type of update (started, completed, feedback)"
    )
...
class RevelationProgressSync(BaseModel):
    ...
    partner_progress: Dict[str, Any] = Field(
        ..., description="Partner's revelation progress"
    )
...
class PersonalizedRevelationTheme(BaseModel):
    ...
    personalization_score: float = Field(
        ..., ge=0.0, le=1.0, description="How well theme fits user"
    )
    compatibility_requirement: float = Field(
        ..., ge=0.0, le=1.0, description="Required compatibility level"
    )
...
class RevelationMatchingInsight(BaseModel):
    ...
    compatibility_score: float = Field(
        ..., ge=0.0, le=1.0, description="Revelation compatibility score"
    )
    shared_themes: List[str] = Field(
        ..., description="Themes both users respond well to"
    )
    complementary_themes: List[str] = Field(
        ..., description="Themes that complement each other"
    )
    communication_sync: float = Field(
        ..., ge=0.0, le=1.0, description="How well communication styles align"
    )
    timing_compatibility: float = Field(
        ..., ge=0.0, le=1.0, description="How well timing preferences align"
    )
    recommendations: List[str] = Field(
        ..., description="Recommendations for improving revelation experience"
    )
...
class AdaptiveRevelationConfig(BaseModel):
    ...
    communication_style_override: Optional[str] = Field(
        default=None, description="Override communication style"
    )
    timing_preferences: Optional[Dict[str, Any]] = Field(
        default=None, description="Timing preferences"
    )
    depth_progression_rate: str = Field(
        default="normal",
        description="How quickly to increase depth (slow, normal, fast)",
    )
    enable_follow_ups: bool = Field(
        default=True, description="Whether to generate follow-up suggestions"
    )
    enable_timing_optimization: bool = Field(
        default=True, description="Whether to optimize timing"
    )
...
class RevelationEngagementMetrics(BaseModel):
    ...
    time_to_start: Optional[float] = Field(
        default=None, description="Time to start writing (seconds)"
    )
    completion_time: Optional[float] = Field(
        default=None, description="Time to complete (seconds)"
    )
    word_count: int = Field(default=0, description="Word count of revelation")
    engagement_indicators: Dict[str, Any] = Field(
        default={}, description="Various engagement metrics"
    )
    follow_up_engagement: Optional[Dict[str, Any]] = Field(
        default=None, description="Follow-up interaction metrics"
    )
    partner_response_time: Optional[float] = Field(
        default=None, description="Partner's response time"
    )
    mutual_engagement_score: Optional[float] = Field(
        default=None, description="Combined engagement score"
    )
...
class RevelationA11ySupport(BaseModel):
    ...
    interaction_alternatives: Optional[Dict[str, str]] = Field(
        default=None, description="Alternative interaction methods"
    )
    reading_level: Optional[str] = Field(
        default=None, description="Reading level assessment"
    )
...
class RevelationLocalization(BaseModel):
    ...
    cultural_sensitivity_notes: Optional[List[str]] = Field(
        default=None, description="Cultural sensitivity considerations"
    )
    regional_preferences: Optional[Dict[str, Any]] = Field(
        default=None, description="Regional customizations"
    )
