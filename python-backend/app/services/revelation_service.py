...
class RevelationService:
    ...
    def can_user_share_revelation(
        self, db: Session, connection_id: int, user_id: int, day: int
    ) -> Dict[str, Any]:
        ...
        if day > current_day:
            return {
                "can_share": False,
                "reason": f"Day {day} not yet unlocked. Current day: {current_day}",
            }
    ...
    def get_connection_revelations(
        self, db: Session, connection_id: int, user_id: int
    ) -> Dict[str, Any]:
        ...
        revelations = (
            db.query(DailyRevelation)
            .filter(DailyRevelation.connection_id == connection_id)
            .order_by(DailyRevelation.day_number.asc())
            .all()
        )
        ...
        for day in range(1, 8):  # Days 1-7
            ...
            user_revelation = next(
                (r for r in day_revelations if r.sender_id == user_id), None
            )
            partner_revelation = next(
                (r for r in day_revelations if r.sender_id != user_id), None
            )
            ...
            timeline_entry = {
                ...
                "user_shared_at": (
                    user_revelation.created_at.isoformat() if user_revelation else None
                ),
                "partner_shared_at": (
                    partner_revelation.created_at.isoformat()
                    if partner_revelation
                    else None
                ),
            }
    ...
    def calculate_completion_percentage(self, timeline: List[Dict]) -> float:
        ...
        return (
            round((completed / total_possible * 100), 1) if total_possible > 0 else 0.0
        )

    def check_photo_reveal_eligibility(
        ...
        if current_day < 7:
            return {
                "eligible": False,
                "reason": f"Photo reveal unlocks on day 7. Current day: {current_day}",
            }
        ...
        user1_revelations = len(
            [r for r in revelations if r.sender_id == connection.user1_id]
        )
        user2_revelations = len(
            [r for r in revelations if r.sender_id == connection.user2_id]
        )

        min_revelations_required = 4  # At least 4 out of 6 days

        if (
            user1_revelations < min_revelations_required
            or user2_revelations < min_revelations_required
        ):
            return {
                "eligible": False,
    ...
    def handle_photo_reveal_consent(
        self, db: Session, connection_id: int, user_id: int, consent: bool
    ) -> Dict[str, Any]:
    ...
    def get_revelation_statistics(self, db: Session, user_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_revelations_per_connection": round(
                total_revelations_shared / max(total_connections, 1), 1
            ),
        }
