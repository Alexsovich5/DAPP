...
class AdaptiveRevelationEngine:
    ...
    async def generate_adaptive_revelation_prompts(
        ...
        try:
            ...
            context = await self._build_revelation_context(
                user, connection, revelation_day, db
            )
    ...
    async def _build_revelation_context(
        ...
        personalization_profile = (
            await personalization_engine.get_or_create_personalization_profile(
                user.id, db
            )
        )
        ...
        partner_id = (
            connection.user2_id
            if connection.user1_id == user.id
        ...
        previous_revelations = (
            ...
            .order_by(DailyRevelation.day_number.desc())
            .limit(5)
            .all()
        )
        ...
        flow_analytics = (
            db.query(ConversationFlowAnalytics)
            .filter(
                and_(
            ...
            .order_by(ConversationFlowAnalytics.analysis_date.desc())
            .first()
        )
        ...
        if variation_index == 0:
            ...
            if total_weight > 0:
                ...
                return random.choices(
                    [theme for theme, _ in top_themes], weights=weights
                )[0]
            else:
    ...
    async def _personalize_revelation_template(
        ...
        for variable in base_template.get("variables", []):
            ...
            metadata["personalization_factors"].append(
                {"variable": variable, "value": replacement}
            )
        ...
        personalized_text = self._add_contextual_elements(personalized_text, context)
        ...
        if variable == "topic":
            ...
            if theme == "passions_and_dreams":
                ...
                topics = [
                    ...
                    "an experience that taught you something important",
                ]
            ...
            if user_interests:
                relevant_interests = [
                    interest for interest in user_interests if len(interest) > 3
                ]
                if relevant_interests:
                    topics.extend(
                        [
                            f"your interest in {interest}"
                            for interest in relevant_interests[:2]
                        ]
                    )
    ...
    def _add_contextual_elements(self, text: str, context: Dict[str, Any]) -> str:
        """
        ...
        if connection_stage == "deeper_connection" and revelation_day >= 4:
            if "connection" not in text.lower():
                text = (
                    text
                    + " I feel like we're really starting to understand each other."
                )

        return text

    def _calculate_prompt_confidence(
        self, personalized_prompt: Dict[str, Any], context: Dict[str, Any]
    ) -> float:
        """
        ...
        personalization_factors = personalized_prompt["metadata"].get(
            "personalization_factors", []
        )
    ...
    async def _analyze_user_revelation_patterns(
        ...
        return {
            ...
            "engagement_trend": (
                "increasing" if len(revelations) >= 10 else "developing"
            ),
        }
    ...
    def _get_fallback_prompts(
        self, revelation_day: int, count: int
    ) -> List[Dict[str, Any]]:
        """
