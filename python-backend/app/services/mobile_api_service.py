...
class MobileAPIService:
    ...
    def detect_mobile_client(
        ...
        try:
            ...
            if device_type == MobileDeviceType.IOS:
                try:
                    os_version = (
                        user_agent_lower.split("os ")[1].split()[0].replace("_", ".")
                    )
                except IndexError:
                    pass
    ...
    def get_optimization_config(
        self, client_info: MobileClientInfo
    ) -> OptimizationConfig:
        ...
        if client_info.network_type in self.network_configs:
            ...
            config.cache_ttl = int(
                config.cache_ttl * network_config["cache_ttl_multiplier"]
            )
    ...
    async def optimize_profile_response(
        self, profile_data: Dict[str, Any], client_info: MobileClientInfo
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # Create cache key
            cache_key = self._generate_profile_cache_key(
                profile_data["user_id"], client_info, config
            )
    ...
    async def optimize_matches_response(
        self, matches_data: List[Dict[str, Any]], client_info: MobileClientInfo
    ) -> List[Dict[str, Any]]:
    ...
    async def optimize_discovery_response(
        self, discovery_data: Dict[str, Any], client_info: MobileClientInfo
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if "users" in discovery_data:
                ...
                for user in discovery_data["users"][:10]:  # Limit for mobile
                    optimized_user = await self._optimize_discovery_user_data(
                        user, config
                    )
            ...
            discovery_data["mobile_pagination"] = {
                ...
                "load_more_url": (
                    "/api/v1/mobile/discovery/next"
                    if len(discovery_data.get("users", [])) >= 10
                    else None
                ),
            }
    ...
    async def _optimize_profile_data(
        self, profile_data: Dict[str, Any], config: OptimizationConfig
    ) -> Dict[str, Any]:
        ...
        if config.response_format == ResponseFormat.MINIMAL:
            ...
            if "life_philosophy" in optimized:
                optimized["life_philosophy"] = self._truncate_text(
                    optimized["life_philosophy"], 150
                )
    ...
    async def _optimize_match_data(
        self, match_data: Dict[str, Any], config: OptimizationConfig
    ) -> Dict[str, Any]:
    ...
    async def _optimize_discovery_user_data(
        self, user_data: Dict[str, Any], config: OptimizationConfig
    ) -> Dict[str, Any]:
    ...
    async def _optimize_photo_urls(
        self, photos: List[Dict[str, Any]], config: OptimizationConfig
    ) -> List[Dict[str, Any]]:
        ...
        for photo in photos:
            ...
            if config.lazy_loading:
                ...
                optimized_photo["placeholder_url"] = (
                    f"{photo['url']}?w=50&q=30&fm=webp&blur=20"
                )
    ...
    def _calculate_optimized_height(
        self, original_width: int, original_height: int, max_width: int
    ) -> int:
    ...
    def _optimize_last_message(
        self, last_message: Optional[Dict[str, Any]]
    ...
    def compress_response(
        self, response_data: str, compression_type: CompressionType
    ) -> bytes:
    ...
    async def preload_user_data(self, user_id: int, client_info: MobileClientInfo):
        ...
        try:
            ...
            for task in preload_tasks:
                cache_key = (
                    f"mobile:preload:{user_id}:{task}:{client_info.device_type.value}"
                )
                ...
                if not self.redis_client.exists(cache_key):
                    ...
                    await self._cache_response(
                        cache_key, preload_data, config.cache_ttl
                    )
    ...
    async def clear_user_cache(self, user_id: int):
        ...
        try:
            # Clear profile cache
            profile_keys = self.redis_client.keys(
                f"{self.cache_prefixes['profile']}{user_id}:*"
            )
...
def mobile_optimized(cache_ttl: int = 1800):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            client_info = mobile_service.detect_mobile_client(
            ...
            if client_info.device_type != MobileDeviceType.UNKNOWN:
                if isinstance(response_data, dict):
                    ...
                    if "profile" in str(func.__name__).lower():
                        ...
                        response_data = (
                            await mobile_service.optimize_discovery_response(
                                response_data, client_info
                            )
                        )
