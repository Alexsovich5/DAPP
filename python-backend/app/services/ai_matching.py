...
class PrivacyFirstMatchingAI:
    ...
    def __init__(self):
        ...
        self.lda_model = LatentDirichletAllocation(
            n_components=10, random_state=42, max_iter=100
        )
    ...
    async def initialize_models(self, db: Session):
        ...
        try:
            ...
            if len(profiles) < 10:
                logger.warning(
                    "Insufficient data for AI model initialization. Using basic algorithms."
                )
    ...
    async def analyze_communication_compatibility(
    ...
    async def analyze_emotional_compatibility(self, user1: User, user2: User) -> float:
        ...
        try:
            ...
            emotional_style_score = self.compare_emotional_styles(
        ...
        for trait1, desc1 in personality1.items():
            for trait2, desc2 in personality2.items():
                ...
                if trait1 == trait2:
                    compatibility_score += 1.0
                # Compatible traits
                elif trait1 in compatible_traits and any(
                    comp in str(desc2).lower() for comp in compatible_traits[trait1]
                ):
    ...
    def are_complementary_traits(self, trait1: str, trait2: str) -> bool:
        ...
        for pair in complementary_pairs:
            if (
                trait1_lower in pair
                and trait2_lower in pair
                and trait1_lower != trait2_lower
            ):
                return True
    ...
    def calculate_goal_alignment(self, goals1: List[str], goals2: List[str]) -> float:
        ...
        common_categories = set(categories1.keys()).intersection(
            set(categories2.keys())
        )
    ...
    async def analyze_personality_compatibility(
        self, user1: User, user2: User
    ) -> float:
        ...
        try:
            ...
            complementarity_score = self.calculate_personality_complementarity(
                traits1, traits2
            )
    ...
    def calculate_personality_complementarity(
        self, traits1: dict, traits2: dict
    ) -> float:
    ...
    async def calculate_comprehensive_compatibility(
        ...
        try:
            ...
            communication_compatibility = (
                await self.analyze_communication_compatibility(user1, user2)
            )
            emotional_compatibility = await self.analyze_emotional_compatibility(
                user1, user2
            )
            life_goals_compatibility = await self.analyze_life_goals_compatibility(
                user1, user2
            )
            personality_compatibility = await self.analyze_personality_compatibility(
                user1, user2
            )
            ...
            overall_score = (
                ...
                + communication_compatibility
                * self.compatibility_weights["communication_style"]
                + emotional_compatibility
                * self.compatibility_weights["emotional_depth"]
                + life_goals_compatibility * self.compatibility_weights["life_goals"]
                + personality_compatibility
                * self.compatibility_weights["personality_match"]
    ...
    async def identify_unique_connection_factors(
        self, user1: User, user2: User
    ) -> List[str]:
        ...
        try:
            ...
            if rare_common:
                unique_factors.extend(
                    [f"shared passion for {interest}" for interest in rare_common]
                )
            ...
            for trait1, trait2 in complementary_pairs:
                if (
                    trait1 in str(traits1).lower() and trait2 in str(traits2).lower()
                ) or (
            ...
            for keyword in deep_value_keywords:
                if any(keyword in str(v).lower() for v in values1.values()) and any(
                    keyword in str(v).lower() for v in values2.values()
                ):
    ...
    async def generate_conversation_starters(
        ...
        try:
            compatibility = await self.calculate_comprehensive_compatibility(
                user1, user2
            )
            starters = []
            ...
            if common_interests:
                ...
                starters.append(
                    f"I noticed we both enjoy {interest}. What drew you to it?"
                )
            ...
            if breakdown.get("life_goals", 0) > 70:
                starters.append(
                    "Our life perspectives seem quite aligned. What's a goal you're excited about right now?"
                )

            # Communication style starters
            if breakdown.get("communication_style", 0) > 70:
                starters.append(
                    "I have a feeling we'd have great conversations. What's something you've been thinking about lately?"
                )
            ...
            if unique_factors:
                ...
                starters.append(
                    f"I'm curious about your perspective on {factor.replace('shared ', '').replace('commitment to ', '')}."
                )

            # Personality-based starters
            if breakdown.get("personality_match", 0) > 60:
                starters.append(
                    "Your approach to life seems really thoughtful. How do you typically navigate important decisions?"
                )

            # Default thoughtful starter
            if not starters:
                starters.append(
                    "I'd love to hear more about what makes you feel most fulfilled in life."
                )
