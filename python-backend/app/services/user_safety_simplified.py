...
class UserSafetyService:
    ...
    def __init__(
        self, database=None, redis_client=None, content_moderation_service=None
    ):
    ...
    async def submit_report(self, report: UserReport) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Safety report {report_id} submitted for user {report.reported_user_id}"
            )
            ...
            if immediate_action["action_required"]:
                await self._take_immediate_action(
                    report.reported_user_id, immediate_action
                )
                logger.warning(
                    f"Immediate action taken for user {report.reported_user_id}: {immediate_action['reason']}"
                )
    ...
    async def _validate_report(self, report: UserReport) -> Dict:
        ...
        recent_reports = [
            ...
            if r["reporter_id"] == report.reporter_id
            and (
                datetime.utcnow() - datetime.fromisoformat(r["timestamp"])
            ).total_seconds()
            < 24 * 3600
        ]
        ...
        if len(recent_reports) > 5:
            return {
                "is_valid": False,
                "error": "Too many reports submitted in the last 24 hours",
            }
    ...
    async def _take_immediate_action(self, user_id: int, action_info: Dict):
        ...
        logger.warning(
            f"Safety action applied to user {user_id}: {action_type.value} - {reason}"
        )
    ...
    async def _take_threshold_action(
        self, user_id: int, category: str, report_count: int
    ):
        ...
        if category == "harassment":
            action_type = ActionType.MESSAGE_RESTRICTION
            reason = (
                f"Multiple harassment reports ({report_count}) - messaging restricted"
            )

        elif category == "fake_profile":
            action_type = ActionType.PROFILE_REVIEW
            reason = (
                f"Multiple fake profile reports ({report_count}) - profile under review"
            )
            ...
            action_type = ActionType.PROFILE_REVIEW
            reason = (
                f"Multiple {category} reports ({report_count}) - account under review"
            )
    ...
    async def get_user_safety_status(self, user_id: int) -> Dict:
        ...
        safety_score = max(
            0, 100 - (len(user_reports) * 10)
        )  # Reduce by 10 for each report
