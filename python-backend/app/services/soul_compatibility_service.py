...
class SoulCompatibilityService:
    ...
    def calculate_compatibility(
        ...
        try:
            ...
            compatibility_summary = self._generate_summary(
                total_score, strengths, growth_areas
            )
    ...
    def _calculate_emotional_resonance(self, user1: User, user2: User) -> float:
        ...
        try:
            ...
            expression_score = self._compare_emotional_expression(
                responses1, responses2
            )
            emotional_factors.append(expression_score)

            return (
                sum(emotional_factors) / len(emotional_factors)
    ...
    def _calculate_lifestyle_compatibility(self, user1: User, user2: User) -> float:
        ...
        try:
            ...
            for pref1, pref2 in incompatible_pairs:
                if (pref1 in diet1 and pref2 in diet2) or (
                    pref2 in diet1 and pref1 in diet2
                ):
                    return 30.0
    ...
    ) -> Tuple[str, ConnectionEnergyLevel]:
    ...
    def _compare_emotional_expression(
    ...
    def _identify_strengths(
        self, user1: User, user2: User, scores: Dict[str, float]
    ) -> List[str]:
    ...
    def _generate_summary(
        self, total_score: float, strengths: List[str], growth_areas: List[str]
    ) -> str:
    ...
    async def track_compatibility_accuracy(
        self,
        connection_id: int,
        predicted_score: CompatibilityScore,
        db: Session,
