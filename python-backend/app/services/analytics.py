...
class AnalyticsService:
        ...
        try:
            ...
            ch_event = {
                ...
                "event_category": (
                    ...
                    if hasattr(event_data["event_category"], "value")
                    else event_data["event_category"]
                ),
            ...
            self.clickhouse.execute("INSERT INTO user_events VALUES", [ch_event])
    ...
    async def _update_real_time_metrics(self, event_data: Dict):
        ...
        try:
            ...
            if event_data["user_id"]:
                ...
                pipe.expire(
                    f"active_users:hourly:{hour_key}", 3600 * 25
                )  # Keep for 25 hours
                pipe.expire(
                    f"active_users:daily:{day_key}", 86400 * 32
                )  # Keep for 32 days
    ...
    async def _update_category_metrics(
        self, event_data: Dict, pipe, hour_key: str, day_key: str
    ):
    ...
    async def track_profile_interaction(
        self,
        viewer_id: int,
        viewed_id: int,
        ...
        await self._store_profile_interaction(
            viewer_id,
            viewed_id,
            interaction_type,
            compatibility_score,
    ...
    async def _store_profile_interaction(
        self,
        viewer_id: int,
        viewed_id: int,
        interaction_type: str,
        ...
        try:
            ...
            self.clickhouse.execute(
                "INSERT INTO profile_interactions VALUES", [interaction_data]
            )

        except Exception as e:
            logger.error(f"Failed to store profile interaction: {e}")

    async def track_match_created(
        self,
        user1_id: int,
        user2_id: int,
        match_type: str,
    ...
    async def _store_matching_event(
        self,
        user1_id: int,
        user2_id: int,
        match_type: str,
        ...
        try:
            ...
            self.clickhouse.execute("INSERT INTO matching_events VALUES", [match_data])

        except Exception as e:
            logger.error(f"Failed to store matching event: {e}")

    async def track_revelation_event(
        self,
        user_id: int,
        match_id: int,
        revelation_day: int,
        ...
        await self._store_revelation_event(
            user_id, match_id, revelation_day, revelation_type, content_length
        )

        return await self.track_event(event)

    async def _store_revelation_event(
        self,
        user_id: int,
        match_id: int,
        revelation_day: int,
        ...
        try:
            ...
            self.clickhouse.execute(
                "INSERT INTO revelation_events VALUES", [revelation_data]
            )
    ...
    async def _calculate_engagement_score(self, user_id: int) -> float:
        ...
        try:
            ...
            interaction_score = (
                matching_events + messaging_events + revelation_events
            ) * 1  # Up to 25 points
