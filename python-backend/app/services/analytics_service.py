...
class AnalyticsService:
    ...
    async def calculate_user_engagement_score(self, user_id: int, db: Session) -> float:
        ...
        try:
            ...
            events = (
                db.query(UserEngagementAnalytics)
                .filter(
            ...
            deep_count = sum(
                1
                for event in events
                if event.event_type in [e.value for e in deep_actions]
            )
    ...
    async def get_user_journey_funnel(
        ...
        try:
            ...
            total_users = db.query(User).filter(User.created_at >= cutoff_date).count()
            ...
            first_swipe_users = (
                ...
                .filter(
                    UserEngagementAnalytics.created_at >= cutoff_date,
                    UserEngagementAnalytics.event_type
                    == AnalyticsEventType.SWIPE_ACTION.value,
                )
                .distinct(UserEngagementAnalytics.user_id)
                .count()
            )
            ...
            first_match_users = (
                ...
                .filter(
                    UserEngagementAnalytics.created_at >= cutoff_date,
                    UserEngagementAnalytics.event_type
                    == AnalyticsEventType.CONNECTION_INITIATED.value,
                )
                .distinct(UserEngagementAnalytics.user_id)
                .count()
            )
            ...
            photo_reveal_users = (
                ...
                .filter(
                    UserEngagementAnalytics.created_at >= cutoff_date,
                    UserEngagementAnalytics.event_type
                    == AnalyticsEventType.PHOTO_CONSENT_GIVEN.value,
                )
                .distinct(UserEngagementAnalytics.user_id)
                .count()
            )
    ...
    async def get_engagement_metrics(self, db: Session) -> EngagementMetrics:
        ...
        try:
            ...
            # Active users
            daily_active = (
                db.query(UserEngagementAnalytics.user_id)
                .filter(UserEngagementAnalytics.created_at >= day_ago)
                .distinct()
                .count()
            )
            ...
            weekly_active = (
                db.query(UserEngagementAnalytics.user_id)
                .filter(UserEngagementAnalytics.created_at >= week_ago)
                .distinct()
                .count()
            )
            ...
            monthly_active = (
                db.query(UserEngagementAnalytics.user_id)
                .filter(UserEngagementAnalytics.created_at >= month_ago)
                .distinct()
                .count()
            )
            ...
            total_sessions = (
                ...
                .filter(
                    UserEngagementAnalytics.event_type
                    == AnalyticsEventType.LOGIN.value,
            ...
            single_session_users = (
                ...
                .group_by(UserEngagementAnalytics.user_id)
                .having(func.count(UserEngagementAnalytics.id) == 1)
                .count()
            )
    ...
    async def get_connection_metrics(self, db: Session) -> ConnectionMetrics:
        ...
        try:
            ...
            active_connections = (
                ...
                .filter(SoulConnection.status == "active")
                .count()
            )
            ...
            photo_reveals = (
                db.query(SoulConnection)
                .filter(SoulConnection.photo_revealed_at.isnot(None))
                .count()
            )

            # Average compatibility score
            avg_compatibility = (
                db.query(func.avg(SoulConnection.compatibility_score))
                .filter(SoulConnection.compatibility_score.isnot(None))
                .scalar()
                or 0.0
            )
            ...
            avg_time_to_message = (
                db.query(
                    func.avg(
                        func.extract(
                            "epoch",
                            SoulConnection.first_message_at - SoulConnection.created_at,
                        )
                    )
                )
                .filter(SoulConnection.first_message_at.isnot(None))
                .scalar()
                or 0.0
            )
            ...
            return ConnectionMetrics(
                ...
                average_time_to_first_message=avg_time_to_message
    ...
    async def get_system_health_metrics(self, db: Session) -> SystemHealthMetrics:
        ...
        try:
            ...
            recent_metrics = (
                db.query(SystemPerformanceMetrics)
                .filter(
                    SystemPerformanceMetrics.measured_at
    ...
    async def generate_user_insights(self, user_id: int, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_connections = len(
                [c for c in connections if c.photo_revealed_at]
            )
    ...
    async def track_system_performance(
        self, metric_name: str, value: float, component: str, db: Session
    ) -> bool:
    ...
    async def update_user_retention_metrics(self, user_id: int, db: Session) -> bool:
        ...
        try:
            ...
            retention = (
                db.query(UserRetentionMetrics)
                .filter(UserRetentionMetrics.user_id == user_id)
                .first()
            )
            ...
            retention.made_first_connection = (
                db.query(SoulConnection)
                .filter(
                    or_(
                        SoulConnection.user1_id == user_id,
            ...
            retention.reached_photo_reveal = (
                db.query(SoulConnection)
                .filter(
                    or_(
                        SoulConnection.user1_id == user_id,
    ...
    async def _update_emotional_journey(
        ...
        try:
            ...
            journey = (
                db.query(EmotionalJourneyTracking)
                .filter(
    ...
    async def _generate_user_recommendations(
        self, user_id: int, events: List, connections: List, db: Session
    ) -> List[str]:
        ...
        if len(events) < 10:
            recommendations.append(
                "Try exploring more features to enhance your experience"
            )

        if not any(
            e.event_type == AnalyticsEventType.REVELATION_SHARED.value for e in events
        ):
            recommendations.append(
                "Share your first soul revelation to deepen connections"
            )
        ...
        if len(active_connections) > 0 and not any(
            c.photo_revealed_at for c in connections
        ):
            recommendations.append(
                "Consider sharing photos after building emotional connection"
            )

        return recommendations[:3]  # Top 3 recommendations

    async def _get_user_milestones(
    ...
    def _empty_engagement_metrics(self) -> EngagementMetrics:
        return EngagementMetrics(
            0, 0, 0, 0, 0, 0, {"day_1": 0, "day_7": 0, "day_30": 0}
        )
