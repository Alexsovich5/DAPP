...
class PredictiveAnalyticsService:
    ...
    async def predict_match_success(
        self, user1_id: int, user2_id: int, compatibility_score: float
    ) -> PredictionResult:
        """
        ...
        try:
            ...
            success_probability = model.predict_proba(combined_features.reshape(1, -1))[
                0
            ][1]
            ...
            logger.error(
                f"Failed to predict match success for users {user1_id}, {user2_id}: {e}"
            )
            raise

    async def predict_conversation_likelihood(
        ...
        try:
            ...
            conv_features = self._create_conversation_features(
                user1_features, user2_features
            )

            model = self.models[PredictionType.CONVERSATION_LIKELIHOOD]
            conversation_probability = model.predict_proba(
                conv_features.reshape(1, -1)
            )[0][1]
    ...
    async def predict_subscription_propensity(self, user_id: int) -> PredictionResult:
        ...
        try:
            ...
            feature_vector = self._prepare_features(
                features, PredictionType.SUBSCRIPTION_PROPENSITY
            )

            model = self.models[PredictionType.SUBSCRIPTION_PROPENSITY]
            subscription_probability = model.predict_proba(
            ...
            logger.error(
                f"Failed to predict subscription propensity for user {user_id}: {e}"
            )
            raise
    ...
    async def get_user_engagement_score(self, user_id: int) -> float:
        ...
        try:
            ...
            engagement_score = (
                min(
                    features.login_frequency * 10, 30
                )  # Login frequency (max 30 points)
                + min(features.match_rate * 100, 25)  # Match rate (max 25 points)
                + min(
                    features.conversation_rate * 100, 25
                )  # Conversation rate (max 25 points)
                + min(
                    features.profile_completeness * 20, 20
            ...
            logger.error(
                f"Failed to calculate engagement score for user {user_id}: {e}"
            )
            return 0.0
    ...
    async def _train_model(self, prediction_type: PredictionType):
        ...
        try:
            ...
            if training_data.empty:
                logger.warning(
                    f"No training data available for {prediction_type.value}"
                )
                self._create_dummy_model(prediction_type)
                return
            ...
            self.model_versions[prediction_type] = (
    ...
    def _prepare_features(
        self, features: UserFeatures, prediction_type: PredictionType
    ) -> np.ndarray:
        """
        ...
        if prediction_type in self.scalers:
            feature_array = self.scalers[prediction_type].transform(
    ...
    def _create_match_features(
        ...
        combined_features = [
            compatibility_score,
            (user1_features.profile_completeness + user2_features.profile_completeness)
            / 2,
            ...
            (
                1 if user1_features.gender != user2_features.gender else 0
            ),  # Different genders
        ]

        return np.array(combined_features)

    def _create_conversation_features(
        self, user1_features: UserFeatures, user2_features: UserFeatures
    ) -> np.ndarray:
        """
        ...
        conv_features = [
            (
                user1_features.compatibility_scores_avg
                + user2_features.compatibility_scores_avg
            )
            / 2,
            (user1_features.match_rate + user2_features.match_rate) / 2,
            (
                user1_features.avg_response_time_hours
                + user2_features.avg_response_time_hours
            )
            / 2,
            (user1_features.profile_completeness + user2_features.profile_completeness)
            / 2,
            (user1_features.total_conversations + user2_features.total_conversations)
            / 2,
    ...
    async def _store_prediction(self, prediction: PredictionResult):
        ...
        try:
            ...
            prediction_key = (
                f"prediction:{prediction.user_id}:{prediction.prediction_type.value}"
            )
            prediction_data = {
