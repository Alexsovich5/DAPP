...
class ElasticsearchService:
    ...
    async def _create_profiles_index(self):
        ...
        try:
            await self.es_client.indices.create(
                index=self.indexes["profiles"], body=mapping, ignore=400
            )
        except Exception as e:
    ...
    async def _create_activities_index(self):
        ...
        try:
            await self.es_client.indices.create(
                index=self.indexes["activities"], body=mapping, ignore=400
            )
        except Exception as e:
            logger.warning(f"Activities index creation warning: {e}")

    async def search_users(
        self, searcher_id: int, criteria: SearchCriteria
    ) -> SearchResponse:
        """
    ...
    async def _build_search_query(
        ...
        query["query"]["bool"]["must_not"].append({"term": {"user_id": searcher_id}})
                ...
                [
                    {
                        "terms": {
                            ...
                            "boost": self.search_configs["boost_factors"][
                                "mutual_interests"
                            ],
                        }
                    }
                            ...
                            "query": {
                                "bool": {
                                    "must": [
                                        ...
                                        {
                                            "range": {
                                                "personality_traits.score": {"gte": 0.6}
                                            }
                                        },
                                    ]
                                }
    ...
    async def _apply_advanced_filters(self, query: Dict, filters: Dict[str, Any]):
        ...
        if "premium_only" in filters and filters["premium_only"]:
            query["query"]["bool"]["filter"].append({"term": {"is_premium": True}})
            ...
            query["query"]["bool"]["filter"].append(
                {
                    "range": {
                        "profile_completeness": {
                            "gte": filters["min_profile_completeness"]
                        }
                    }
                }
            )
    ...
    async def _apply_sorting(self, query: Dict, criteria: SearchCriteria):
        ...
        if criteria.sort_by == SortOrder.COMPATIBILITY_SCORE:
            ...
            query["sort"] = [{"last_active": {"order": "desc"}}, "_score"]

        elif criteria.sort_by == SortOrder.NEWEST_FIRST:
            query["sort"] = [{"created_at": {"order": "desc"}}, "_score"]
    ...
    async def _wrap_with_function_score(
        self, query: Dict, searcher_profile: Dict, criteria: SearchCriteria
    ) -> Dict[str, Any]:
        ...
        functions.append(
            {
                "field_value_factor": {
                    ...
                    "factor": self.search_configs["boost_factors"][
                        "profile_completeness"
                    ],
                    "modifier": "sqrt",
                    "missing": 0.1,
            ...
            functions.append(
                {
                    ...
                    "weight": self.search_configs["boost_factors"][
                        "location_proximity"
                    ],
                }
            )
    ...
    async def _process_search_results(
        self, searcher_id: int, response: Dict, criteria: SearchCriteria
    ) -> List[SearchResult]:
        ...
        for hit in response["hits"]["hits"]:
            ...
            if searcher_location and source.get("location"):
                try:
                    ...
                    distance_km = round(
                        geopy.distance.distance(searcher_point, user_point).kilometers,
                        1,
                    )
                except Exception as e:
            ...
            result = SearchResult(
                ...
                last_active=datetime.fromisoformat(
                    source["last_active"].replace("Z", "+00:00")
                ),
            )
    ...
    async def _calculate_compatibility_score(
        ...
        try:
            ...
            cache_key = (
                f"compatibility:{min(user1_id, user2_id)}:{max(user1_id, user2_id)}"
            )
    ...
    async def _get_user_profile(self, user_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            response = await self.es_client.get(
                index=self.indexes["profiles"], id=user_id, ignore=404
            )
    ...
    async def _generate_search_suggestions(
        self, criteria: SearchCriteria, response: Dict
    ) -> List[str]:
    ...
    async def delete_user(self, user_id: int) -> bool:
        ...
        try:
            for index in self.indexes.values():
                await self.es_client.delete(index=index, id=user_id, ignore=404)
            return True
        except Exception as e:
...
def init_search_service(
    elasticsearch_client: AsyncElasticsearch, redis_client: redis.Redis
