...
class UserSafetyService:
    ...
    async def _check_immediate_action_required(self, report: UserReport) -> Dict:
        ...
        pending_critical = await self._get_pending_critical_reports(
                ...
                actions_to_take.append(
                    {
                        ...
                        "action": self._determine_action_for_category(
                            category, len(reports)
                        ),
                    }
                )
    ...
    async def _analyze_report(self, report: Dict) -> Dict:
        ...
        analysis = {
            ...
            "credibility_score": await self._calculate_reporter_credibility(
                report["reporter_id"]
            ),
        }
        ...
        if "message_ids" in report["evidence"]:
            ...
            analysis["content_analysis"] = await self._analyze_reported_messages(
                message_ids
            )
        ...
        if analysis["credibility_score"] > 0.8:
            ...
            if category == ReportCategory.HARASSMENT:
                if (
                    ...
                    and analysis["content_analysis"]["harassment_score"] > 0.7
                ):
                    decision.update(
                        {
                            "action_required": True,
    ...
    async def get_user_safety_status(self, user_id: int) -> Dict:
        ...
        return {
            ...
            "risk_level": self._assess_user_risk_level(
                safety_score, len(recent_reports)
            ),
