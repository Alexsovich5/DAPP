...
class PushNotificationService:
    ...
    def __init__(
        ...
        self.notification_templates = {
            ...
            NotificationType.PHOTO_REVEAL: {
                ...
                "actions": [{"action": "view_photos", "title": "View Photos"}],
                "vibrate": [500],
                "require_interaction": True,
            },
    ...
    async def send_notification(
        ...
        try:
            ...
            await self.log_notification(
                user_id, notification_type, payload, success_count > 0, db
            )
    ...
    async def send_to_subscription(
        ...
        try:
            ...
            if e.response and e.response.status_code in [404, 410]:
                ...
                db.commit()
                logger.info(
                    f"Deactivated invalid subscription for user {subscription.user_id}"
                )
    ...
    def create_notification_payload(
        self, notification_type: NotificationType, context: Dict[str, Any]
    ) -> NotificationPayload:
    ...
    async def send_bulk_notifications(
        self, notifications: List[Dict[str, Any]], db: Session
    ) -> Dict[str, int]:
    ...
    async def notify_new_revelation(
        ...
        return await self.send_notification(
            ...
            context={
                ...
                "day": day,
                "revelation_preview": (
                    revelation_preview[:40] + "..."
                    if len(revelation_preview) > 40
                    else revelation_preview
                ),
                "connectionId": connection_id,
            },
            db=db,
    ...
    async def notify_photo_reveal(
        self, user_id: int, revealer_name: str, connection_id: int, db: Session
    ) -> bool:
        """Send photo reveal notification"""
        return await self.send_notification(
            user_id=user_id,
            notification_type=NotificationType.PHOTO_REVEAL,
            context={"name": revealer_name, "connectionId": connection_id},
            db=db,
        )
    ...
    async def notify_revelation_reminder(
        self, user_id: int, connection_id: int, day: int, db: Session
    ) -> bool:
        """Send revelation reminder notification"""
        return await self.send_notification(
            user_id=user_id,
            notification_type=NotificationType.REVELATION_REMINDER,
            context={"connectionId": connection_id, "day": day},
            db=db,
        )
    ...
    async def track_notification_click(
        self, user_id: int, notification_id: int, db: Session
    ) -> bool:
    ...
    async def get_notification_analytics(
        self, user_id: int = None, days: int = 30
    ) -> Dict[str, Any]:
    ...
    async def cleanup_old_subscriptions(
        self, days: int = 30, db: Session = None
    ) -> int:
        ...
        try:
            ...
            old_subscriptions = (
                ...
                .filter(
                    ...
                    PushSubscription.is_active is False,
                )
                .all()
            )
