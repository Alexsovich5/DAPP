...
class AIMatchingService:
    ...
    async def generate_user_profile_embeddings(
        ...
        try:
            ...
            personality_analysis = await self._analyze_personality_from_revelations(
                user_id, db
            )
            ...
            communication_vector = await self._generate_communication_embedding(
            ...
            profile.conscientiousness_score = personality_analysis.get(
            ...
            profile.emotional_intelligence = personality_analysis.get(
                "emotional_intelligence", 0.5
            )
            profile.attachment_style = personality_analysis.get(
                "attachment_style", "secure"
            )
            profile.communication_style = communication_analysis.get(
                "style", "balanced"
            )
            profile.conversation_depth_preference = communication_analysis.get(
                "depth_preference", 0.7
            )

            # AI confidence and completeness
            profile.ai_confidence_level = self._calculate_ai_confidence(
                user, personality_analysis, communication_analysis
            )
            profile.profile_completeness_score = self._calculate_profile_completeness(
                user
            )
    ...
    async def calculate_ai_compatibility(
        self, user1_id: int, user2_id: int, db: Session
    ) -> CompatibilityPrediction:
        ...
        try:
            ...
            interests_compat = self._calculate_interests_compatibility(
                profile1, profile2
            )
            ...
            lifestyle_compat = await self._calculate_lifestyle_compatibility(
                user1_id, user2_id, db
            )
            growth_potential = await self._calculate_growth_potential(
                profile1, profile2, db
            )
            conflict_prediction = await self._predict_conflict_likelihood(
                profile1, profile2, db
            )
            ...
            prediction = (
                db.query(CompatibilityPrediction)
                .filter(
                    and_(
            ...
            prediction.conversation_quality_prediction = (
                await self._predict_conversation_quality(profile1, profile2, db)
            )
            ...
            logger.info(
                f"Calculated AI compatibility: {overall_compatibility:.3f} for users {user1_id}-{user2_id}"
            )
            return prediction
    ...
    async def generate_personalized_recommendations(
        self, user_id: int, limit: int = 10, db: Session = None
        ...
        try:
            ...
            existing_connections = (
                db.query(SoulConnection.user1_id, SoulConnection.user2_id)
                .filter(
                    or_(
            ...
            potential_matches = (
                ...
                .filter(
                    and_(
                        ...
                        UserProfile.ai_confidence_level
                        > 0.3,  # Only users with decent AI confidence
                    )
                )
                .limit(50)
                .all()
            ...
            for match_profile in potential_matches:
                ...
                if (
                    compatibility_pred.overall_compatibility
                    ...
                    recommendation = MatchRecommendation(
                        ...
                        conversation_starters=compatibility_pred.conversation_starters
                        or [],
                        predicted_success_rate=compatibility_pred.long_term_potential
                        or 0.5,
            ...
            await analytics_service.track_user_event(
                ...
                event_data={
                    ...
                    "avg_compatibility": (
                        sum(r.compatibility_score for r in final_recommendations)
                    ...
                    "high_strength_count": len(
                        [
                            ...
                            if r.recommendation_strength == "high"
                        ]
                    ),
                },
                db=db,
            ...
            logger.info(
                f"Generated {len(final_recommendations)} AI recommendations for user {user_id}"
            )
    ...
    async def analyze_user_behavior(
        self, user_id: int, days_back: int = 30, db: Session = None
    ) -> BehaviorAnalysis:
        ...
        try:
            ...
            preferences = self._extract_user_preferences(
                messages, revelations, connections
            )
            ...
            logger.info(
                f"Analyzed behavior for user {user_id}: {len(patterns)} patterns detected"
            )
            return analysis
    ...
    async def _analyze_personality_from_revelations(
        ...
        revelations = (
            db.query(DailyRevelation).filter(DailyRevelation.sender_id == user_id).all()
        )
        ...
        personality_scores["openness"] = self._calculate_keyword_score(
            text_content, openness_keywords
        )
        ...
        personality_scores["conscientiousness"] = self._calculate_keyword_score(
            text_content, conscientiousness_keywords
        )
        ...
        personality_scores["extraversion"] = self._calculate_keyword_score(
            text_content, extraversion_keywords
        )
        ...
        personality_scores["agreeableness"] = self._calculate_keyword_score(
            text_content, agreeableness_keywords
        )
        ...
        personality_scores["neuroticism"] = 1.0 - self._calculate_keyword_score(
            text_content, stability_keywords
        )
        ...
        personality_scores["emotional_intelligence"] = self._calculate_keyword_score(
            text_content, eq_keywords
        )
    ...
    async def _analyze_behavioral_patterns(
        ...
        patterns["feature_usage"]["revelations"] = safe_int(getattr(user, 'total_revelations_shared', 0))
        patterns["feature_usage"]["swipes"] = safe_int(getattr(user, 'total_swipes', 0))
        patterns["feature_usage"]["matches"] = safe_int(getattr(user, 'total_matches', 0))
    ...
    async def _generate_personality_embedding(
        self, personality_analysis: Dict[str, Any]
    ) -> List[float]:
        ...
        for category in interest_categories:
            ...
            for interest in interests:
                if category.lower() in str(interest).lower():
                    ...
                    break
            embedding.extend(
                [score, score * 0.9, score * 1.1, score * 0.7]
            )  # 4 dims per category
    ...
    async def _generate_communication_embedding(
        self, communication_analysis: Dict[str, Any]
    ) -> List[float]:
    ...
    async def _ensure_user_profile(self, user_id: int, db: Session) -> UserProfile:
        ...
        if not profile:
            ...
            not profile.last_updated_by_ai
            or (datetime.utcnow() - profile.last_updated_by_ai).days
            > self.profile_update_interval_days
        ):
    ...
    def _calculate_personality_compatibility(
        ...
        if isinstance(profile1, dict) and isinstance(profile2, dict):
            ...
            if not hasattr(profile1, 'personality_vector') or not hasattr(profile2, 'personality_vector'):
                return 0.5
    ...
    def _calculate_interests_compatibility(
        ...
        if isinstance(profile1, list) and isinstance(profile2, list):
            ...
            # Handle UserProfile objects
            if not hasattr(profile1, 'interests_vector') or not hasattr(profile2, 'interests_vector'):
                return 0.5
    ...
    async def _calculate_lifestyle_compatibility(
        ...
        activity_compat = 1.0 - abs(
            behavior1.engagement_score - behavior2.engagement_score
        )

        # Compare communication styles
        style_compat = (
            0.8
            if behavior1.communication_style == behavior2.communication_style
            else 0.5
        )
    ...
    def _calculate_prediction_confidence(
        ...
        combined_confidence = (
            2 * (conf1 * conf2) / (conf1 + conf2) if (conf1 + conf2) > 0 else 0.5
        )
    ...
    async def _calculate_growth_potential(
        self, profile1: UserProfile, profile2: UserProfile, db: Session
    ) -> float:
        ...
        if profile1.conscientiousness_score and profile2.conscientiousness_score:
            consc_avg = (
                profile1.conscientiousness_score + profile2.conscientiousness_score
            ) / 2
        ...
        if profile1.emotional_intelligence and profile2.emotional_intelligence:
            ei_avg = (
                profile1.emotional_intelligence + profile2.emotional_intelligence
            ) / 2
    ...
    async def _predict_conflict_likelihood(
        ...
        if profile1.neuroticism_score and profile2.neuroticism_score:
            avg_neuroticism = (
                profile1.neuroticism_score + profile2.neuroticism_score
            ) / 2
        ...
        if profile1.agreeableness_score and profile2.agreeableness_score:
            min_agreeableness = min(
                profile1.agreeableness_score, profile2.agreeableness_score
            )
        ...
        if (
            ...
            and profile2.communication_style
            and profile1.communication_style != profile2.communication_style
        ):
            if (
                profile1.communication_style == "direct"
                and profile2.communication_style == "diplomatic"
            ):
    ...
    async def _predict_conversation_quality(
        ...
        if (
            profile1.conversation_depth_preference
            and profile2.conversation_depth_preference
        ):
            depth_similarity = 1.0 - abs(
                profile1.conversation_depth_preference
                - profile2.conversation_depth_preference
            )
    ...
    async def _generate_conversation_starters(
        ...
        if (
            ...
            and profile2.openness_score
            and profile1.openness_score > 0.6
            and profile2.openness_score > 0.6
        ):
            starters.append(
        ...
        if (
            ...
            or profile2.communication_style == "detailed"
        ):
            starters.append(
                "I'd love to hear your thoughts on something you're passionate about."
            )
            starters.append("What's a topic you could talk about for hours?")

        # Based on emotional intelligence
        if (
            profile1.emotional_intelligence
            and profile2.emotional_intelligence
            and (profile1.emotional_intelligence + profile2.emotional_intelligence) / 2
            > 0.7
        ):
    ...
    def _generate_potential_challenges(
        ...
        if (
            ...
            and profile2.neuroticism_score
            and abs(profile1.neuroticism_score - profile2.neuroticism_score) > 0.4
        ):
            challenges.append("Different approaches to handling stress")

        if (
            profile1.conscientiousness_score
            and profile2.conscientiousness_score
            and abs(profile1.conscientiousness_score - profile2.conscientiousness_score)
            > 0.5
        ):
        ...
        if (
            ...
            challenges.append(
                "May need to adapt to different communication preferences"
            )
    ...
    def _calculate_behavioral_engagement_score(
        ...
        if messages:
            ...
            engagement_factors.append(
                min(1.0, msg_per_day / 5.0)
            )  # Normalize to 5 messages/day = 1.0

        # Revelation participation
        if revelations:
            rev_per_week = len(revelations) / (days_back / 7)
            engagement_factors.append(
                min(1.0, rev_per_week / 2.0)
            )  # 2 revelations/week = 1.0
    ...
    def _extract_user_preferences(
        self, messages, revelations, connections
    ) -> Dict[str, Any]:
        ...
        if revelations:
            avg_revelation_length = sum(
                len(r.content or "") for r in revelations
            ) / len(revelations)
    ...
    def _generate_behavioral_recommendations(
        self, patterns, communication_style, engagement_score
    ) -> List[str]:
        ...
        if engagement_score < 0.4:
            recommendations.append(
                "Consider being more active in conversations to improve connections"
            )

        if "slow_responder" in patterns:
            recommendations.append(
                "Try to respond to messages more promptly to maintain connection momentum"
            )

        if "brief_communicator" in patterns:
            recommendations.append(
                "Consider sharing more detailed thoughts to deepen emotional connections"
            )

        if communication_style == "concise":
            recommendations.append(
                "Sometimes longer messages can help build deeper emotional bonds"
            )
