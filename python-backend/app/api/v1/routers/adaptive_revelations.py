...
from app.schemas.adaptive_revelation_schemas import (
    AdaptiveRevelationRequest,
    AdaptiveRevelationResponse,
    RevelationAnalyticsResponse,
    RevelationFeedbackRequest,
    RevelationThemeResponse,
    RevelationTimingResponse,
)
            ...
            theme_responses.append(
                RevelationThemeResponse(
                    ...
                    description=theme_data.get(
                        "description",
                        f"Theme focused on {theme_name.replace('_', ' ')}",
                    ),
                    communication_style=theme_data.get(
                        "communication_style", "balanced"
                    ),
                    requires_high_compatibility=theme_data.get(
                        "requires_high_compatibility", False
                    ),
                    emotional_intensity=adaptive_revelation_engine.depth_progression[
                        revelation_day
                    ]["emotional_intensity"],
                    sample_templates=[
...
async def get_revelation_analytics(
    ...
    try:
        ...
        revelations = (
            ...
            .order_by(DailyRevelation.day_number)
            .all()
        )

        # Analyze revelation patterns
        revelation_patterns = (
            await adaptive_revelation_engine._analyze_user_revelation_patterns(
                current_user.id, db
            )
        )
        ...
        return RevelationAnalyticsResponse(
            ...
            overall_depth_score=sum(rev.day_number for rev in revelations)
            / max(completed_days, 1),
    ...
    try:
        ...
        progress = {
            ...
            "user_progress": {
                ...
                "next_day": (
                    max(user_completed_days, default=0) + 1
                    if len(user_completed_days) < 7
                    else None
                ),
            },
            "partner_progress": {
                ...
                "next_day": (
                    max(partner_completed_days, default=0) + 1
                    if len(partner_completed_days) < 7
                    else None
                ),
            },
            "mutual_completion": {
                "completed_together": sorted(
                    list(user_completed_days.intersection(partner_completed_days))
                ),
                "waiting_for_partner": sorted(
                    list(user_completed_days - partner_completed_days)
                ),
                "waiting_for_user": sorted(
                    list(partner_completed_days - user_completed_days)
                ),
            },
