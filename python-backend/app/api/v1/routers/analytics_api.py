...
async def get_user_engagement_score(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Error calculating engagement score"
        )
...
async def get_dashboard_summary(
    ...
    try:
        ...
        user_insights = await analytics_service.generate_user_insights(
            current_user.id, db
        )
        user_engagement = await analytics_service.calculate_user_engagement_score(
            current_user.id, db
        )
...
async def get_system_health_metrics(
    ...
    try:
        ...
        return {
            ...
            "reliability": {"errorRates": health_metrics.error_rates},
        ...
        raise HTTPException(
            status_code=500, detail="Error getting system health metrics"
        )
...
async def get_realtime_active_users(
    ...
    try:
        ...
        active_count = (
            ...
            .filter(UserEngagementAnalytics.created_at >= recent_cutoff)
            .distinct()
            .count()
        )
