...
async def get_matching_analytics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve matching analytics"
        )
...
async def track_custom_event(
    ...
    try:
        ...
        from ....services.analytics import AnalyticsEvent, EventCategory, EventType
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid event type or category: {e}"
        )
    except Exception as e:
...
async def get_a_b_test_results(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve A/B test results"
        )
...
async def track_a_b_test_event(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to track A/B test event"
            )
...
async def delete_user_analytics_data(
    ...
    try:
        ...
        if success:
            return {
                "status": "success",
                "message": f"Analytics data deleted for user {user_id}",
            }
        else:
...
async def analytics_health_check():
    ...
    try:
        ...
        return {
            "status": "healthy",
            "timestamp": datetime.utcnow(),
            "services": {"clickhouse": "connected", "redis": "connected"},
        }
    except Exception as e:
