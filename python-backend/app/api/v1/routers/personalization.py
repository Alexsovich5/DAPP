...
from app.schemas.personalization_schemas import (
    ...
    RevelationPromptRequest,
    RevelationPromptResponse,
    SmartReplyRequest,
    SmartReplyResponse,
    UIPersonalizationRequest,
...
async def get_personalization_profile(
    ...
    try:
        ...
        logger.error(
            f"Error retrieving personalization profile for user {current_user.id}: {str(e)}"
        )
        raise HTTPException(
...
async def optimize_algorithm_performance(
    ...
    try:
        ...
        optimization_result = (
            await personalization_engine.optimize_algorithm_performance(
    ...
    try:
        content = (
            ...
            .filter(
                ...
                PersonalizedContent.user_profile_id
                == current_user.personalization_profile.id,
            )
            .first()
        )
...
async def get_conversation_flow_analytics(
    ...
    try:
        ...
        if connection_id:
            query = query.filter(
                ConversationFlowAnalytics.connection_id == connection_id
            )

        analytics = (
            query.order_by(ConversationFlowAnalytics.analysis_date.desc())
            .limit(10)
            .all()
        )
...
async def update_personalization_preferences(
    ...
    try:
        ...
        allowed_fields = {
            ...
            "revelation_timing_preference",
            "content_depth_preference",
            "preferred_ui_complexity",
...
async def get_optimization_history(
    ...
    try:
        ...
        if optimization_type:
            query = query.filter(
                AlgorithmOptimization.optimization_type == optimization_type
            )

        optimizations = (
            query.order_by(AlgorithmOptimization.created_at.desc()).limit(limit).all()
        )
