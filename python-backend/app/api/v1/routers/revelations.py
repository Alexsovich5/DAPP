...
def create_revelation(
    ...
    try:
        ...
        logger.info(
            f"New revelation created: User {current_user.id}, Connection {revelation_data.connection_id}, Day {revelation_data.day_number}"
        )
...
def get_revelation_timeline(
    ...
    try:
        ...
        revelations = (
            ...
            .order_by(DailyRevelation.day_number, DailyRevelation.created_at)
            .all()
        )
        ...
        for day in range(1, 8):
            user_revelation = next(
                (
                    ...
                    if r.day_number == day and r.sender_id == current_user.id
                ),
                None,
            )
            partner_revelation = next(
                (
                    r
                    for r in revelations
                    if r.day_number == day and r.sender_id == partner_id
                ),
                None,
            )
            ...
            timeline_days.append(
                {
                    ...
                    "userSharedAt": (
                        user_revelation.created_at.isoformat()
                        if user_revelation
                        else None
                    ),
                    "partnerSharedAt": (
        ...
        user_shared_count = len(
            [r for r in revelations if r.sender_id == current_user.id]
        )
        partner_shared_count = len(
            [r for r in revelations if r.sender_id == partner_id]
        )
        mutual_days = len(
            set(r.day_number for r in revelations if r.sender_id == current_user.id)
    ...
    try:
        ...
        return {
            ...
            "message": (
                "Photos revealed!"
                if mutual_consent
                else "Waiting for partner's consent"
            ),
        }
...
def get_revelation_analytics(
    ...
    try:
        ...
        analytics = (
            db.query(SoulConnectionAnalytics)
            .filter(SoulConnectionAnalytics.connection_id == connection_id)
            .first()
        )
