...
async def generate_ai_profile(
    ...
    try:
        ...
        return AIProfileResponse(
            ...
            last_updated=(
                profile.last_updated_by_ai.isoformat()
                if profile.last_updated_by_ai
...
async def get_ai_match_recommendations(
    ...
    try:
        ...
        recommendations = (
            await ai_matching_service.generate_personalized_recommendations(
        ...
        for rec in recommendations:
            ...
            recommended_user = (
                db.query(User).filter(User.id == rec.recommended_user_id).first()
            )
            ...
            if recommended_user:
                user_profile_data = {
                    ...
                    "age": (
                        _calculate_age(recommended_user.date_of_birth)
                        if recommended_user.date_of_birth
                        else None
                    ),
...
def _get_trait_suggestions(trait: str, score: float) -> List[str]:
    ...
    suggestions = {
        "openness": {
            "low": [
                "Try exploring new hobbies",
                "Read books from different genres",
                "Visit museums or art galleries",
            ],
            "high": [
                "Channel creativity into projects",
                "Share your ideas with others",
                "Embrace structured approaches sometimes",
            ],
        },
        "conscientiousness": {
            "low": [
                "Use planning apps or calendars",
