...
class NotificationRequest(BaseModel):
    ...
    context: Optional[Dict[str, Any]] = Field(
        None, description="Notification context data"
    )
...
async def update_notification_preferences(
    ...
    try:
        ...
        success = await push_service.update_notification_preferences(
            user_id=current_user.id, preferences=prefs_dict
        )
...
async def track_notification_click(
    ...
    try:
        ...
        success = await push_service.track_notification_click(
            user_id=current_user.id, notification_id=notification_id, db=db
        )
...
async def get_user_subscriptions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get subscriptions: {str(e)}"
        )
...
async def test_notification(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Test notification failed: {str(e)}"
        )
