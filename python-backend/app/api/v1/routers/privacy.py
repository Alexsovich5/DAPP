...
class DataSubjectRightsRequest(BaseModel):
    ...
    data_categories: List[str] = Field(
        default=[], description="Specific data categories (optional)"
    )
...
async def grant_consent(
    ...
    try:
        result = await privacy_service.collect_user_consent(
            user_id=current_user.id, consent_categories=request.categories
        )
...
async def withdraw_consent(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid category or purpose: {str(e)}"
        )
    except Exception as e:
...
async def get_privacy_dashboard(
    ...
    """
    try:
        dashboard_data = await privacy_service.get_privacy_dashboard_data(
            current_user.id
        )
        ...
        logger.error(
            f"Error fetching privacy dashboard for user {current_user.id}: {str(e)}"
        )
    ...
    try:
        ...
        for cat in request.data_categories:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid data category: {cat}"
                )
        ...
        logger.error(
            f"Error processing data subject request for user {current_user.id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500, detail="Failed to process data subject request"
        )
...
async def download_data_export(
    ...
    try:
        ...
        logger.error(
            f"Error downloading data export {request_id} for user {current_user.id}: {str(e)}"
        )
...
async def update_privacy_preferences(
    ...
    try:
        ...
        logger.error(
            f"Error updating privacy preferences for user {current_user.id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500, detail="Failed to update privacy preferences"
        )
...
async def request_data_rectification(
    ...
    try:
        ...
        logger.error(
            f"Error processing rectification request for user {current_user.id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500, detail="Failed to process rectification request"
        )
...
async def get_privacy_requests(
    ...
    try:
        ...
        logger.error(
            f"Error fetching privacy requests for user {current_user.id}: {str(e)}"
        )
...
async def get_privacy_policy():
    ...
    try:
        ...
        return {"success": True, "privacy_policy": privacy_policy}
