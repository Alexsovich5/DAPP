...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_users_profile_complete ON users (is_profile_complete)"
    )

    # Composite indexes for common matching queries
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_users_active_location ON users (is_active, location)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_users_active_age ON users (is_active, date_of_birth)"
    )

    # Soul connection performance indexes
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_soul_connections_user1 ON soul_connections (user1_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_soul_connections_user2 ON soul_connections (user2_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_soul_connections_status ON soul_connections (status)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_soul_connections_stage ON soul_connections (connection_stage)"
    )

    # Composite indexes for connection queries
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_soul_connections_user1_status ON soul_connections (user1_id, status)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_soul_connections_user2_status ON soul_connections (user2_id, status)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_soul_connections_users_status ON soul_connections (user1_id, user2_id, status)"
    )

    # Message performance indexes
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_messages_connection_id ON messages (connection_id)"
    )
    op.execute(
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_messages_connection_created ON messages (connection_id, created_at)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_messages_sender_unread ON messages (sender_id, is_read)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_messages_connection_unread ON messages (connection_id, is_read)"
    )

    # Revelation performance indexes
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_revelations_connection_id ON daily_revelations (connection_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_revelations_sender_id ON daily_revelations (sender_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_revelations_day_number ON daily_revelations (day_number)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_revelations_created_at ON daily_revelations (created_at)"
    )

    # Composite indexes for revelation queries
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_revelations_connection_day ON daily_revelations (connection_id, day_number)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_revelations_sender_day ON daily_revelations (sender_id, day_number)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_revelations_connection_sender ON daily_revelations (connection_id, sender_id)"
    )
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_matches_receiver_id ON matches (receiver_id)"
    )
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_matches_sender_status ON matches (sender_id, status)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_matches_receiver_status ON matches (receiver_id, status)"
    )
